...
export const decimalsToFractions: FiltersMap['decimalsToFractions'] = num => {
    ...
    const integer = Math.trunc(absNum);
    const frac = absNum - integer;
    ...
    if (bestDelta <= tolerance) {
        const whole = integer ? `${integer} ` : '';
        return `${sign}${whole}${best.glyph}`;
    }

    // Fallback: rounded to 2 decimal places, keep sign
